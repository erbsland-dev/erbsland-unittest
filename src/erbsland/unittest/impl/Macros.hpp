...
#define ASSERT_CONTEXT_REQUIRE_FOR_TEST(test, macroName, flags, ...)                                                   \
    ::erbsland::unittest::require(test, flags, macroName, #__VA_ARGS__, {__FILE__, __LINE__}, [&]() -> bool {          \
        return static_cast<bool>(__VA_ARGS__);                                                                         \
    });
#define ASSERT_CONTEXT_COMPARISON(macroName, flags, op, a, b)                                                          \
    ::erbsland::unittest::requireComparison(                                                                           \
        this,                                                                                                          \
        flags,                                                                                                         \
        macroName,                                                                                                     \
        #a #op #b,                                                                                                     \
        {__FILE__, __LINE__},                                                                                          \
        [&]() -> bool { return static_cast<bool>((a)op(b)); },                                                         \
        [&]() -> std::string { return ::erbsland::unittest::comparisonErrorMessage(#op, #a, #b, (a), (b)); });
#define ASSERT_CONTEXT_COMPARISON_FOR_TEST(test, macroName, flags, op, a, b)                                           \
    ::erbsland::unittest::requireComparison(                                                                           \
        test,                                                                                                          \
        flags,                                                                                                         \
        macroName,                                                                                                     \
...
#define ASSERT_CONTEXT_THROWS_AS(macroName, flags, exceptionClass, ...)                                                \
    ::erbsland::unittest::requireThrowsAs<exceptionClass>(                                                             \
        this, flags, macroName, #exceptionClass ", " #__VA_ARGS__, {__FILE__, __LINE__}, [&]() -> void {               \
            static_cast<void>(__VA_ARGS__);                                                                            \
        });
...
#define REQUIRE_THROWS_AS(exceptionClass, ...)                                                                         \
    ASSERT_CONTEXT_THROWS_AS("REQUIRE_THROWS_AS", 0, exceptionClass, __VA_ARGS__)
...
#define CHECK_FALSE(...)                                                                                               \
    ASSERT_CONTEXT_REQUIRE(                                                                                            \
        "CHECK_FALSE", (::erbsland::unittest::AssertNegate | ::erbsland::unittest::AssertCheck), __VA_ARGS__)
#define CHECK_THROWS(...) ASSERT_CONTEXT_THROWS("CHECK_THROWS", (::erbsland::unittest::AssertCheck), __VA_ARGS__)
#define CHECK_THROWS_AS(exceptionClass, ...)                                                                           \
    ASSERT_CONTEXT_THROWS_AS("CHECK_THROWS_AS", (::erbsland::unittest::AssertCheck), exceptionClass, __VA_ARGS__)
...
#define CHECK_NOT_EQUAL(a, b)                                                                                          \
    ASSERT_CONTEXT_COMPARISON("CHECK_NOT_EQUAL", (::erbsland::unittest::AssertCheck), !=, a, b)
#define CHECK_LESS(a, b) ASSERT_CONTEXT_COMPARISON("CHECK_LESS", (::erbsland::unittest::AssertCheck), <, a, b)
#define CHECK_LESS_EQUAL(a, b)                                                                                         \
    ASSERT_CONTEXT_COMPARISON("CHECK_LESS_EQUAL", (::erbsland::unittest::AssertCheck), <=, a, b)
...
#define CHECK_GREATER_EQUAL(a, b)                                                                                      \
    ASSERT_CONTEXT_COMPARISON("CHECK_GREATER_EQUAL", (::erbsland::unittest::AssertCheck), >=, a, b)
...
#define TESTS_ENTRY(function_name) std::make_tuple((&function_name), #function_name),

/// Add a test to the current unit test class.
/// Use this method in the constructor of your unit test to add all test methods.
#define ADD_TEST(function_name) p.addTest((&function_name), std::string(#function_name));
