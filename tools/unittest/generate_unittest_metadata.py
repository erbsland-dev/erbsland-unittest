...
class WorkingSet:
    ...
    RE_TEST_METHOD = re.compile(
        R"""(?x)
        (   # Capture one or more tags for the test method.
            (?: [A-Z_]{4,20} \( [^)]* \) \s* )+
        )?
        void \s+
        (   # Accept test methods starting with `test` or `print`.
            (?: test | print ) \w+
        ) \s* \( \s* \)
        """,
        re.DOTALL,
    ...
    def parse_cmd_line(self):
        ...
        """
        parser = argparse.ArgumentParser(description="Build unittest metadata.")
        ...
        paths_to_check: list[Path] = list(self._paths_to_check(paths_from_input))
    ...
    def _paths_to_check(self, paths_from_input: list[Path]):
