...
from lib.config import project_relative_posix, read_elcl_file, validate_file_suffixes, validate_local_names
from lib.error import UtilityError
from lib.file_update import FileUpdate
from lib.path_safety import read_safe_bytes, read_safe_text, require_directory, resolve_project_path
...
class SecurityHashesApp(UtilityApp):
    ...
    def add_command_line_args(self, parser: argparse.ArgumentParser) -> None:
        parser.add_argument("--verify", action="store_true", help="Verify the manifest instead of writing it.")
    ...
    def create_records(self) -> list[HashRecord]:
        ...
        if self.config is None:
            raise UtilityError("Missing configuration.")
        records = [
            HashRecord(self.config.display_path(directory, is_directory=True), self.hash_directory(directory))
    ...
    def verify_manifest(self, manifest: str) -> None:
        ...
        if self.config is None:
            raise UtilityError("Missing configuration.")
        existing = read_safe_text(self.config.output_file, "Security hash manifest", FileUpdate.MAX_COMPARE_FILE_SIZE)
