...
from lib.safe_tool import SafeExecutable, find_safe_executable, safe_subprocess_environment
    ...
    def read(cls, project_dir: Path, config_file: Path) -> "GitHubWorkflowsConfig":
        ...
        result = cls(
            ...
            workflow_directory=resolve_project_path(
                project_dir, main_config.get_text("workflow_directory"), "Workflow Directory"
            ),
...
class WorkflowFile:
    ...
    def references(self) -> list[ActionReference]:
        ...
        for index, line in enumerate(self.lines):
            ...
            if "@" not in value:
                references.append(
                    ActionReference(self.path, index, match.group("indent"), match.group("prefix"), value, value, "")
                )
                continue
            ...
            repo = "/".join(parts[:2])
            references.append(
                ActionReference(self.path, index, match.group("indent"), match.group("prefix"), action, repo, ref)
            )
        return references
...
class GitHubWorkflowsApp(UtilityApp):
    ...
    def process_reference(
        self, workflow: WorkflowFile, reference: ActionReference, tags: GitTagCache | None, violations: list[str]
    ) -> ActionStatus | None:
        ...
        if latest is not None and latest.version > tag.version:
            return ActionStatus(f"{reference.action}@{tag.version}", f"latest {latest.version}", workflow_name)
    ...
    def process_workflows(self, git: SafeExecutable) -> None:
        ...
        for workflow in workflows:
            references = sorted(workflow.references(), key=lambda reference: reference.line_index, reverse=True)
